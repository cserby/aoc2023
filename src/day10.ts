...
function nextState(state: State, map: Map): State | undefined {
  ...
  return { pos: nPos, heading: nHeading, distance: state.distance + 1 };
}

export function findDistances(map: Map): Distances {
  ...
  const needToVisit: State[] = [
    nextState({pos: map.start, heading: "E", distance: 0}, map),
    nextState({pos: map.start, heading: "W", distance: 0}, map),
    nextState({pos: map.start, heading: "S", distance: 0}, map),
    nextState({pos: map.start, heading: "N", distance: 0}, map),
...
export function day10part1(input: string): number {
  return Math.max(...(findDistances(parseInput(input)).flatMap((l) => l.flatMap(c => c)).filter((c) => c!== Infinity)) as number[]);
}
