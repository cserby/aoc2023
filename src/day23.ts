...
export function neighborFields(pos: Position, snowIsland: SnowIsland): Position[] {
  const neighborFields = [];
  if (pos.line > 0 && snowIsland[pos.line - 1][pos.char] !== "v") neighborFields.push({ line: pos.line - 1, char: pos.char });
  if (pos.char > 0 && snowIsland[pos.line][pos.char - 1] !== ">") neighborFields.push({ line: pos.line, char: pos.char - 1 });
  if (pos.line < snowIsland.length - 1 && snowIsland[pos.line + 1][pos.char] !== "^") neighborFields.push({ line: pos.line + 1, char: pos.char });
  if (pos.char < snowIsland[0].length - 1 && snowIsland[pos.line][pos.char + 1] !== "<") neighborFields.push({ line: pos.line, char: pos.char + 1 });
  return neighborFields.filter((p) => snowIsland[p.line][p.char] !== "#");
}

export function longestPath(snowIsland: SnowIsland): Position[] {
  ...
  function longestPathRec(nextPos: Position, pathSoFar: Position[]): Position[] | undefined {
    if (JSON.stringify(nextPos) === JSON.stringify(finishPos)) {
      ...
      const pathsFromHere =
        neighborFields(nextPos, snowIsland)
